...
var promisify = function(func) {
  return function(num) {
    var myPromise = new Promise(null, waiting);
    func(num, function(err, success) {
      if (err && myPromise.status === waiting) {
        myPromise.status = rejected;
        myPromise.value = err;
      } else if (success && myPromise.status === waiting) {
        myPromise.status = resolved;
        myPromise.value = success;
      }
      if (myPromise.status !== waiting) {
        myPromise.then();
      }
    });
    return myPromise;
